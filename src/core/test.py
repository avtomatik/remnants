...
from core.transform import transform_sub_special, transform_sub_sum
...
def test_data_capital_combined_archived():
    ...
    df_control = pd.concat(
        map(
            lambda _: read_usa_bea_excel(*_, SH_NAME_CONTROL).loc[
                :, SERIES_IDS
            ],
...
def test_data_consistency_d(
    ...
    ).pipe(transform_sub_sum).iloc[:, [-1]].dropna(axis=0).pipe(
...
def test_usa_bea_fixed_assets():
    ...
    SERIES_ID = ["k3n31gd1es00"]
    SERIES_IDS = ["k3n31gd1eq00", "k3n31gd1ip00", "k3n31gd1st00"]
    ...
    # =========================================================================
    test_usa_bea_sfat_series_ids().pipe(transform_sub_special).iloc[
        :, [-1]
    ].dropna(axis=0).pipe(autocorrelation_plot)
...
def test_usa_bea_sfat_series_ids(
    ...
    df_test = stockpile(enlist_series_ids(SERIES_ID + SERIES_IDS, URL.FIAS))
