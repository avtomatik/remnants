...
from core.constants import (SERIES_IDS_CB, SERIES_IDS_CD, SERIES_IDS_COL,
                            SERIES_IDS_PRCH)
from core.funcs import (construct_usa_hist_deflator, enlist_series_ids,
                        stockpile, transform_mean)
...
plt.figure(2)
plt.plot(
    stockpile(enlist_series_ids(SERIES_IDS_COL, Dataset.USCB)).pct_change(),
...
plt.plot(df.loc[:, LABEL].pct_change(), label=LABEL)
plt.title(
    "Fixed Assets Increment, Cobb$-$Douglas, Census 1949 & 1975 Versions"
)
plt.xlabel("Period")
plt.ylabel("Index")
plt.legend()
